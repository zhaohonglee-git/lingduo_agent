...
> **[警告]** 控制器必须已安装 RobotWare **Paint 选项 (687-1 PaintWare)**，否则 PaintL/PaintC 指令会报「未知指令」。

本目录包含以下文件，需要全部传到控制器：
- `DoorPaint_IRC5P.mod` — 主程序模块
...
2. Add Controller → Virtual Controller → 选含 IRC5P 对应 RobotWare 的系统
...
3. `cd /HOME` → `put DoorPaint_IRC5P.mod` → `put BASE.sys` → `put T_ROB1.pgf`
4. 在 FlexPendant 上 Reload

---

## 上线前必做的 4 件事
1. **TCP 标定**：用 4 点法重新标定 `tSprayGun`，把结果填到 `DoorPaint_IRC5P.mod` 顶部
